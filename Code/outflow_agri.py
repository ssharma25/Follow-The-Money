...
def clean_send_to_DB(state,money):
        ...
        """insertDBOutflow(state,np.mean(money_dup))"""
...
"""UPDATE to find average
update ftm.outflow SET state = subquery.state,funds = subquery.funds FROM (select state, avg(funds) AS funds from ftm.outflow GROUP BY 1) AS subquery where ftm.outflow.state = subquery.state;


To create new table of distinct elements
CREATE TABLE ftm.outflowFunds AS SELECT DISTINCT * FROM ftm.outflow;"""
